...
rule all:
    input: 
        ...
        expand('results/humann3GTDB/{samples}_genefamilies.tsv', samples = SAMPLES)
...
rule humann3GTDB:
    input:
        KDRs = 'KDRs/{samples}_kneaddata.fastq'
    output:
        geneFamilies = 'results/humann3GTDB/{samples}_genefamilies.tsv',
        pathwaysCoverage = 'results/humann3GTDB/{samples}_pathcoverage.tsv',
        pathways = 'results/humann3GTDB/{samples}_pathabundance.tsv'
    log:
        'logs/humann3GTDB/{samples}.humann3.log'
    conda:
        'envs/humann3.yaml'
    threads: 10
    resources: 
